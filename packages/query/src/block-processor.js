...
import { getExchangeRateFromValidatorInfoResponse, getIdentityKeyFromValidatorInfoResponse, } from '@penumbra-zone/getters/validator-info-response';
...
import { identifyTransactions } from './helpers/identify-txs.js';
const isSwapRecordWithSwapCommitment = (r) => r instanceof SwapRecord && r.swapCommitment instanceof StateCommitment;
const isSpendableNoteRecordWithNoteCommitment = (r) => r instanceof SpendableNoteRecord && r.noteCommitment instanceof StateCommitment;
...
export class BlockProcessor {
    ...
    constructor({ indexedDb, viewServer, querier, numeraires, stakingAssetId, genesisBlock, walletCreationBlockHeight, }) {
    ...
    sync = () => (this.syncPromise ??= backOff(() => this.syncAndStore(), {
    ...
    stop = (r) => this.abortController.abort(`Sync stop ${r}`);
    setNumeraires(numeraires) {
        this.numeraires = numeraires;
    }
    /**
    ...
    async syncAndStore() {
        ...
        let latestKnownBlockHeight = await backOff(async () => {
        ...
        if (currentHeight === PRE_GENESIS_SYNC_HEIGHT) {
            ...
            if (this.genesisBlock?.height === currentHeight + 1n) {
                ...
                const skipTrialDecrypt = shouldSkipTrialDecrypt(this.walletCreationBlockHeight, currentHeight);
    ...
    async processBlock({ compactBlock, latestKnownBlockHeight, skipTrialDecrypt = false, }) {
        ...
        const spentNullifiers = await this.resolveNullifiers(compactBlock.nullifiers, compactBlock.height);
        ...
        if (spentNullifiers.size || recordsByCommitment.size) {
            ...
            const { relevantTxs, recoveredSourceRecords } = await identifyTransactions(spentNullifiers, recordsByCommitment, blockTx, addr => this.viewServer.isControlledAddress(addr));
        ...
        const blockInPriceRelevanceThreshold = compactBlock.height >= latestKnownBlockHeight - BigInt(PRICE_RELEVANCE_THRESHOLDS.default);
        ...
        if (blockInPriceRelevanceThreshold && compactBlock.swapOutputs.length) {
            await updatePricesFromSwaps(this.indexedDb, this.numeraires, compactBlock.swapOutputs, compactBlock.height);
        }
     ...
     */
    async assertRootValid(blockHeight) {
        ...
        if (remoteRoot.equals(inMemoryRoot)) {
            console.debug(`Block height: ${blockHeight} root matches remote ✅ \n`, `Hash: ${uint8ArrayToHex(inMemoryRoot.inner)}`);
        }
        else {
            console.warn(`Block height: ${blockHeight} root does not match remote ❌ \n`, `Local hash: ${uint8ArrayToHex(inMemoryRoot.inner)} \n`, `Remote hash: ${uint8ArrayToHex(remoteRoot.inner)}`);
        }
    }
    async saveRecoveredCommitmentSources(recovered) {
    ...
    async saveAndReturnMetadata(assetId) {
    ...
    async saveAndReturnDelegationMetadata(identityKey) {
    ...
    async resolveNullifiers(nullifiers, height) {
        const spentNullifiers = new Set();
        const readOperations = [];
        const writeOperations = [];
        for (const nullifier of nullifiers) {
            const readPromise = (async () => {
                const record = (await this.indexedDb.getSpendableNoteByNullifier(nullifier)) ??
     ...
     */
    async identifyAuctionNfts(action) {
        if (action.case === 'actionDutchAuctionSchedule' && action.value.description) {
            ...
            await processActionDutchAuctionWithdraw(action.value.auctionId, action.value.seq, this.indexedDb);
        }
    }
    /**
     * during wasm tx info generation later, wasm independently queries idb for
     ...
     * - update idb
     */
    async identifyLpNftPositions(action) {
        if (action.case === 'positionOpen' && action.value.position) {
            ...
            await this.indexedDb.addPosition(computePositionId(action.value.position), action.value.position);
        }
        if (action.case === 'positionClose' && action.value.positionId) {
            await this.indexedDb.updatePosition(action.value.positionId, new PositionState({ state: PositionState_PositionStateEnum.CLOSED }));
        }
    ...
    async maybeUpsertAuctionWithNoteCommitment(spendableNoteRecord) {
    ...
    async saveTransactions(height, relevantTx) {
        for (const { id, data } of relevantTx) {
            await this.indexedDb.saveTransaction(id, height, data);
        }
    }
    async handleEpochTransition(endHeightOfPreviousEpoch, latestKnownBlockHeight) {
        ...
        const nextEpochIsLatestKnownEpoch = sctParams && latestKnownBlockHeight - nextEpochStartHeight < sctParams.epochDuration;
    ...
    async updateValidatorInfos(nextEpochStartHeight) {
        ...
        for await (const validatorInfoResponse of this.querier.stake.allValidatorInfos()) {
            ...
            await this.updatePriceForValidatorDelegationToken(validatorInfoResponse, nextEpochStartHeight);
        }
    }
    async updatePriceForValidatorDelegationToken(validatorInfoResponse, nextEpochStartHeight) {
        ...
        if (metadata) {
            ...
            await this.indexedDb.updatePrice(assetId, this.stakingAssetId, toDecimalExchangeRate(exchangeRate), nextEpochStartHeight);
        }
    }
}
