...
import { Action, Transaction, TransactionBody, } from '@penumbra-zone/protobuf/penumbra/core/transaction/v1/transaction_pb';
import { BLANK_TX_SOURCE, getCommitmentsFromActions, getNullifiersFromActions, identifyTransactions, } from './identify-txs.js';
import { Output, OutputBody, Spend, SpendBody, } from '@penumbra-zone/protobuf/penumbra/core/component/shielded_pool/v1/shielded_pool_pb';
import { Swap, SwapBody, SwapClaim, SwapClaimBody, } from '@penumbra-zone/protobuf/penumbra/core/component/dex/v1/dex_pb';
import { SpendableNoteRecord } from '@penumbra-zone/protobuf/penumbra/view/v1/view_pb';
import { Any } from '@bufbuild/protobuf';
import { FungibleTokenPacketData, IbcRelay, } from '@penumbra-zone/protobuf/penumbra/core/component/ibc/v1/ibc_pb';
...
describe('identifyTransactions', () => {
    test('returns empty arrays when no relevant transactions are found', async () => {
        ...
        const commitmentRecords = new Map();
        const result = await identifyTransactions(spentNullifiers, commitmentRecords, blockTx, () => false);
    ...
    test('identifies ibc relays', async () => {
        ...
        const spentNullifiers = new Set();
        const commitmentRecords = new Map();
        const result = await identifyTransactions(spentNullifiers, commitmentRecords, blockTx, addr => addr.equals(new Address(addressFromBech32m(knownAddr))));
...
const createIbcRelay = (receiver) => {
    const tokenPacketData = new FungibleTokenPacketData({ receiver });
    const encoder = new TextEncoder();
    const relevantRelay = Any.pack(new MsgRecvPacket({
