...
import { ActionDutchAuctionEnd, AuctionId, DutchAuction, } from '@penumbra-zone/protobuf/penumbra/core/component/auction/v1/auction_pb';
...
vi.mock('@penumbra-zone/wasm/auction', () => ({
    getAuctionNftMetadata: () => Metadata.fromJson({
...
describe('processActionDutchAuctionEnd()', () => {
    ...
    it('saves metadata for the ended auction NFT', async () => {
        await processActionDutchAuctionEnd(action, auctionQuerier, indexedDb);
        expect(indexedDb.saveAssetsMetadata).toHaveBeenCalledWith(expect.objectContaining({ display: 'test' }));
    });
    it('upserts the auction with the sequence number', async () => {
        ...
        auctionQuerier.auctionStateById.mockResolvedValueOnce(new DutchAuction({
            state: {
