...
import { getDelta1Amount, getDelta2Amount, getLambda1Amount, getLambda2Amount, getSwapAsset1, getSwapAsset2, getUnfilled1Amount, getUnfilled2Amount, } from '@penumbra-zone/getters/batch-swap-output-data';
/**
 *
 ...
 */
export const calculatePrice = (delta, unfilled, lambda) => {
...
export const updatePricesFromSwaps = async (indexedDb, numeraires, swapOutputs, height) => {
 ...
 */
export const deriveAndSavePriceFromBSOD = async (indexedDb, numeraireAssetId, swapOutputs, height) => {
    for (const swapOutput of swapOutputs) {
        ...
        if (swapAsset2.equals(numeraireAssetId)) {
            ...
            numerairePerUnit = calculatePrice(getDelta1Amount(swapOutput), getUnfilled1Amount(swapOutput), getLambda2Amount(swapOutput));
        }
        // case for trading pair <numéraire,pricedAsset>
            ...
            numerairePerUnit = calculatePrice(getDelta2Amount(swapOutput), getUnfilled2Amount(swapOutput), getLambda1Amount(swapOutput));
        }
