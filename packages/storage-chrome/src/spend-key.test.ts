...
import { mockLocalExtStorage, mockSessionExtStorage } from './mock';

const localExtStorage = mockLocalExtStorage();
const sessionExtStorage = mockSessionExtStorage();

vi.doMock('./local', async importOriginal => {
  const mod = await importOriginal<typeof import('./defaults')>();
  return {
    ...mod,
    localExtStorage,
  };
...
vi.doMock('./session', async importOriginal => {
  const mod = await importOriginal<typeof import('./session')>();
  return {
    ...mod,
    sessionExtStorage,
  };
...
const { getSpendKey } = await import('./wallet');
...
describe('Authorize request handler', () => {
  ...
  test('should fail if no wallet is present', async () => {
    ...
    await expect(getSpendKey()).rejects.toThrow('Unable to get spend key');
  });
