...
export function getCustodyImplName<T extends string, J = unknown>(
  custodyJson: Record<T, J>,
): Extract<T, CustodyImplName> {
  const [custodyType, ...extra] = Object.keys(custodyJson) as T[];

  if (custodyType == null) {
    throw new RangeError('No custody fields', { cause: custodyJson });
  } else if (extra.length) {
    throw new RangeError(`Additional custody fields ${extra.join()}`, { cause: custodyJson });
  }
