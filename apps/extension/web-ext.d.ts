...
declare module 'web-ext' {
  ...
  type WebExtTarget = 'firefox-desktop' | 'firefox-android' | 'chromium';
  ...
  type WebExtRunCommandOptions = {
     ...
     */
    target?: WebExtTarget | WebExtTarget[];
  ...
  /**
   * Options used by the `web-ext` CLI.
   * @see https://extensionworkshop.com/documentation/develop/getting-started-with-web-ext
   */
  type WebExtConfig = WebExtSharedOptions & {
    /**
     * Options used only by the `web-ext build` command.
     */
    build: WebExtBuildCommandOptions;

    /**
     * Options used only by the `web-ext sign` command.
     */
    sign: WebExtSignCommandOptions;

    /**
     * Options used only by the `web-ext run` command.
     */
    run: WebExtRunCommandOptions;

    /**
     * Options used only by the `web-ext lint` command.
     */
  ...
  type ExtensionRunnerReloadResult = {
    runnerName: string;
    reloadError?: Error;
    sourceDir?: string;
  };
  ...
  export const cmd: {
    run: (opt: WebExtSharedOptions & WebExtRunCommandOptions) => Promise<IExtensionRunner>;
  };
}
